...
const PostForm = () => {
    ...
    return (
        <div className='w-4/5 max-w-4xl p-6 mx-auto mt-24 mb-6 border-2 border-blue-600 rounded-lg bg-zinc-200 sm:mt-14'>
            <form className='flex flex-col justify-center gap-6 p-2 sm:p-4' onSubmit={handleSubmit(onSubmit)}>
                ...
                <button type='onSubmit' className='p-4 text-white bg-blue-700 rounded-lg'>Add Post</button>
            </form>
            <section className='w-full h-auto max-w-md mx-auto my-2'>
                {isPending && <span className='font-bold text-blue-500'>Sending data...</span>}
                {isError && <span className='font-bold text-red-400'>{error.message}</span>}
                {isSuccess && <span className='font-bold text-green-500'>Post added successfully!</span>}
            </section>
        </div>
    )
}
